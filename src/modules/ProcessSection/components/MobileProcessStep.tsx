export const MobileProcessSteps = () => {
  return (
    <div className="box-border caret-transparent gap-x-[60px] flex flex-col min-h-auto min-w-auto gap-y-[60px] mt-5 md:gap-x-[50px] xl:hidden md:min-h-0 md:min-w-0 md:gap-y-[50px] md:mt-0">
      <div className="relative items-start box-border caret-transparent gap-x-3 flex flex-col justify-between min-h-auto min-w-auto gap-y-3 z-3 mb-5 md:items-center md:justify-normal md:min-h-0 md:min-w-0 md:mb-0">
        <div className="text-white text-sm items-center bg-[linear-gradient(rgb(24,25,41),rgb(2,2,5))] shadow-[rgba(86,86,229,0.2)_0px_0px_45.1px_0px] box-border caret-transparent flex h-10 justify-center leading-[22.4px] min-h-auto min-w-auto w-10 border border-gray-800 ml-0 mt-10 p-2 rounded-[100px] border-solid md:text-lg md:h-[70px] md:leading-[28.8px] md:ml-[-5px] md:min-h-0 md:min-w-0 md:w-[70px] md:mt-12 md:p-0">
          01
        </div>
        <div className="items-center box-border caret-transparent gap-x-1.5 flex flex-col justify-between mt-[-63px] max-w-full min-h-auto min-w-auto gap-y-1.5 text-center w-full py-0 rounded-[20px] md:items-center md:gap-x-[27px] md:max-w-[350px] md:min-h-0 md:min-w-0 md:gap-y-[27px] md:text-start md:mt-0 md:py-[30px]">
          <div className="items-center bg-transparent bg-[radial-gradient(circle_at_50%_0%,rgba(23,23,39,0.38),rgb(9,9,9)_100%,rgb(255,255,255))] box-border caret-transparent gap-x-[15px] flex flex-col justify-end min-h-auto min-w-auto gap-y-[15px] text-center border border-zinc-900 ml-7 mb-5 p-5 rounded-[20px] border-solid md:[align-items:normal] md:bg-transparent md:bg-none md:gap-x-[normal] md:block md:flex-row md:justify-normal md:min-h-0 md:min-w-0 md:gap-y-[normal] md:text-start md:ml-0 md:mb-0 md:p-0 md:rounded-none md:border-0 md:border-none md:border-white/60">
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-start">
              <div className="items-center box-border caret-transparent gap-x-[17px] flex flex-row justify-start gap-y-[17px] text-center md:items-center  md:gap-x-[21px] md:flex-col md:justify-between md:gap-y-[21px] md:text-start">
                <div className="text-xs bg-indigo-600 shadow-[rgba(0,0,0,0.12)_0px_4.433px_8.865px_0px] box-border caret-transparent hidden tracking-[-0.569px] leading-[18.6px] text-center uppercase px-[13px] py-2 rounded-[8.8px] md:text-[14.235px] md:flex md:leading-[22.0643px] md:text-start">
                  <div className="text-xs box-border caret-transparent leading-[18.6px] text-center md:text-[14.235px] md:leading-[22.0643px] md:text-start">
                    Ideation
                  </div>
                </div>
                <h3 className="text-white text-[21px] font-medium box-border caret-transparent tracking-[-1.12px] leading-[25.2px] min-h-auto min-w-auto text-center capitalize font-secondary md:text-[34px] md:font-semibold md:leading-[44.2px] md:min-h-0 md:min-w-0 md:text-start">
                  Idea Analysis
                </h3>
              </div>
            </div>
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-center">
              <p className="text-white/50 box-border caret-transparent leading-[20.58px] max-w-[238px] text-center w-full mb-2.5">
                Generate views and sales with our expert team, who have worked
                with the biggest creators.
              </p>
            </div>
          </div>
        </div>
        <div className="relative items-center box-border caret-transparent gap-x-2.5 flex h-auto justify-center max-w-full min-h-auto min-w-auto object-cover gap-y-2.5 w-[400px] z-3 left-[0%] md:gap-x-0 md:h-[284px] md:min-h-0 md:min-w-0 md:gap-y-0 md:z-1 md:left-auto">
          <div className="relative items-center box-border caret-transparent flex h-[300px] justify-center mt-[-31px] min-h-auto min-w-auto w-auto md:items-end md:h-auto md:min-h-0 md:min-w-[500px] md:w-[500px] md:mt-0">
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36b3452df31baf9345_Glow.avif"
              sizes="(max-width: 726px) 100vw, 726px"
              alt=""
              className="box-border caret-transparent h-full max-w-full min-h-auto min-w-auto object-cover w-full md:min-h-0 md:min-w-0"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6c9a7b71a60f057a75d5a_tag_3.avif"
              alt=""
              className="absolute shadow-[rgba(54,108,255,0.57)_0px_9px_20px_-2px] box-border caret-transparent h-[55px] max-w-full rounded-[100px] left-[8%] top-[34%] md:h-[60px] md:left-[17%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6c9a706e0b3217385d044_tag_4.avif"
              alt=""
              className="absolute shadow-[rgba(54,108,255,0.57)_0px_9px_20px_-2px] box-border caret-transparent h-[55px] max-w-full rotate-[-14.99999492786973deg] z-4 rounded-[100px] left-[52%] top-[32%] md:h-[60px] md:transform-none"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6c9a7492d6fd645d90cf9_tag_2.avif"
              alt=""
              className="absolute shadow-[rgba(54,108,255,0.57)_0px_9px_20px_-2px] box-border caret-transparent h-[55px] max-w-full rotate-[-17.99999174234843deg] z-3 rounded-[100px] left-[13%] top-[58%] md:h-[60px] md:transform-none md:left-[10%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6c9a77447cd58110ed2b8_tag_1.avif"
              alt=""
              className="absolute shadow-[rgba(54,108,255,0.57)_0px_9px_20px_-2px] box-border caret-transparent h-[55px] max-w-full rotate-[17.99999174234843deg] z-1 rounded-[100px] left-[54%] top-[58%] md:h-[60px] md:transform-none"
            />
          </div>
        </div>
      </div>
      <div className="relative items-start box-border caret-transparent gap-x-3 flex flex-col justify-between min-h-auto min-w-auto gap-y-3 z-3 md:items-center md:justify-normal md:min-h-0 md:min-w-0">
        <div className="text-white text-sm items-center bg-[linear-gradient(rgb(24,25,41),rgb(2,2,5))] shadow-[rgba(86,86,229,0.2)_0px_0px_45.1px_0px] box-border caret-transparent flex h-10 justify-center leading-[22.4px] min-h-auto min-w-auto w-10 border border-gray-800 ml-0 mt-0 p-2 rounded-[100px] border-solid md:text-lg md:h-[70px] md:leading-[28.8px] md:ml-[-5px] md:min-h-0 md:min-w-0 md:w-[70px] md:mt-10 md:p-0">
          02
        </div>
        <div className="items-center box-border caret-transparent gap-x-1.5 flex flex-col justify-between mt-[-63px] max-w-full min-h-auto min-w-auto gap-y-1.5 text-center w-full py-0 rounded-[20px] md:items-center md:gap-x-[27px] md:max-w-[350px] md:min-h-0 md:min-w-0 md:gap-y-[27px] md:text-start md:mt-0 md:py-[30px]">
          <div className="items-center bg-transparent bg-[radial-gradient(circle_at_50%_0%,rgba(23,23,39,0.38),rgb(9,9,9)_100%,rgb(255,255,255))] box-border caret-transparent gap-x-[15px] flex flex-col justify-end min-h-auto min-w-auto gap-y-[15px] text-center border border-zinc-900 ml-7 mb-5 p-5 rounded-[20px] border-solid md:[align-items:normal] md:bg-transparent md:bg-none md:gap-x-[normal] md:block md:flex-row md:justify-normal md:min-h-0 md:min-w-0 md:gap-y-[normal] md:text-start md:ml-0 md:mb-0 md:p-0 md:rounded-none md:border-0 md:border-none md:border-white/60">
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-start">
              <div className="items-center box-border caret-transparent gap-x-[17px] flex flex-row justify-start gap-y-[17px] text-center md:items-center md:gap-x-[21px] md:flex-col md:justify-between md:gap-y-[21px] md:text-start">
                <div className="text-xs bg-indigo-600 shadow-[rgba(0,0,0,0.12)_0px_4.433px_8.865px_0px] box-border caret-transparent hidden tracking-[-0.569px] leading-[18.6px] text-center uppercase px-[13px] py-2 rounded-[8.8px] md:text-[14.235px] md:flex md:leading-[22.0643px] md:text-start">
                  <div className="text-xs box-border caret-transparent leading-[18.6px] text-center md:text-[14.235px] md:leading-[22.0643px] md:text-start">
                    Scripting
                  </div>
                </div>
                <h3 className="text-white text-[21px] font-medium box-border caret-transparent tracking-[-1.12px] leading-[25.2px] min-h-auto min-w-auto text-center capitalize font-secondary md:text-[34px] md:font-semibold md:leading-[44.2px] md:min-h-0 md:min-w-0 md:text-start">
                  Writing Content
                </h3>
              </div>
            </div>
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-center">
              <p className="text-white/50 box-border caret-transparent leading-[20.58px] max-w-[238px] text-center w-full mb-2.5">
                Generate views and sales with our expert team, who have worked
                with the biggest creators.
              </p>
            </div>
          </div>
        </div>
        <div className="relative items-start bottom-[-50px] box-border caret-transparent gap-x-2.5 flex h-auto justify-center max-w-full min-h-auto min-w-auto gap-y-2.5 w-auto z-1 left-[10%] top-[0%] md:gap-x-0 md:h-[500px] md:justify-start md:min-h-0 md:min-w-0 md:gap-y-0 md:top-[-4%] md:w-[400px] md:left-auto md:bottom-auto">
          <div className="relative items-center box-border caret-transparent flex h-[300px] justify-center min-h-auto min-w-[300px] w-[400px] md:items-end md:h-auto md:min-h-0 md:min-w-[450px] md:w-[600px]">
            <div className="absolute box-border caret-transparent top-[-8%] z-6 left-[6%] md:left-[-7%] md:z-3 md:top-[23%]">
              <div className="relative items-center box-border caret-transparent gap-x-2.5 flex justify-center gap-y-2.5 top-[-6%] left-[0%]">
                <div className="items-center box-border caret-transparent gap-x-[15px] flex justify-center min-h-auto min-w-auto gap-y-[15px] px-10 py-5 md:min-h-0 md:min-w-0">
                  <img
                    src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6a0fc18b1b753b873e667_tag.svg"
                    alt=""
                    className="box-border caret-transparent h-[50px] max-w-full min-h-auto min-w-auto object-cover w-[50px] md:min-h-0 md:min-w-0"
                  />
                  {/*<div className="box-border caret-transparent gap-x-3 flex flex-col h-[23px] min-h-auto min-w-auto gap-y-3 overflow-hidden md:min-h-0 md:min-w-0">
                    <div className="relative items-center box-border caret-transparent flex h-25px-border min-h-auto min-w-auto text-left md:min-h-0 md:min-w-0">
                      <div className="text-white text-2xl box-border caret-transparent leading-[38.4px] min-h-auto min-w-auto md:min-h-0 md:min-w-0">
                        Hook
                      </div>
                    </div>
                    <div className="relative items-center box-border caret-transparent flex h-25px-border min-h-auto min-w-auto text-left md:min-h-0 md:min-w-0">
                      <div className="text-white text-2xl box-border caret-transparent leading-[38.4px] min-h-auto min-w-auto md:min-h-0 md:min-w-0">
                        Tittle
                      </div>
                    </div>
                    <div className="relative items-center box-border caret-transparent flex h-25px-border min-h-auto min-w-auto text-left md:min-h-0 md:min-w-0">
                      <div className="text-white text-2xl box-border caret-transparent leading-[38.4px] min-h-auto min-w-auto md:min-h-0 md:min-w-0">
                        Value
                      </div>
                    </div>
                    <div className="relative items-center box-border caret-transparent flex h-25px-border min-h-auto min-w-auto text-left md:min-h-0 md:min-w-0">
                      <div className="text-white text-2xl box-border caret-transparent leading-[38.4px] min-h-auto min-w-auto md:min-h-0 md:min-w-0">
                        CTA
                      </div>
                    </div>
                    <div className="relative items-center box-border caret-transparent flex h-25px-border min-h-auto min-w-auto text-left md:min-h-0 md:min-w-0">
                      <div className="text-white text-2xl box-border caret-transparent leading-[38.4px] min-h-auto min-w-auto md:min-h-0 md:min-w-0">
                        Hook
                      </div>
                    </div>
                  </div>*/}
                  <div className="bg-white box-border caret-transparent h-[50px] min-h-auto min-w-auto w-px md:min-h-0 md:min-w-0"></div>
                </div>
                <img
                  src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6daa5744c735ff068b073_input_box.png"
                  alt=""
                  className="absolute box-border caret-transparent h-full max-w-full w-full inset-[0%]"
                />
              </div>
            </div>
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36b3452df31baf9345_Glow.avif"
              sizes="(max-width: 726px) 100vw, 726px"
              alt=""
              className="relative box-border caret-transparent h-[120%] max-w-[120%] min-h-auto min-w-auto object-cover translate-y-[-52px] w-[120%] z-2 md:min-h-0 md:min-w-0 md:transform-none"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd3525079b2a2e62ac95_card_1.avif"
              alt=""
              className="absolute shadow-[rgba(64,53,216,0.52)_0px_-5px_20px_-20px] box-border caret-transparent max-w-[250px] object-cover w-full z-3 rounded-[9px] left-[11%] right-[0%] bottom-[16%] md:max-w-[350px] md:rounded-[20px] md:bottom-[7%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6e7100cf992b2952b2345_link_bar.avif"
              alt=""
              className="absolute box-border caret-transparent hidden max-w-[350px] object-cover right-[-16%] w-full z-3 top-[10%] md:block md:top-[19%]"
            />
            <div className="absolute bg-[linear-gradient(rgba(255,255,255,0)_37%,rgb(9,9,9)_100%,rgb(17,17,17))] box-border caret-transparent h-full w-full z-99 inset-[0%] md:h-[110%]"></div>
          </div>
        </div>
      </div>
      <div className="relative items-start box-border caret-transparent gap-x-3 flex flex-col justify-between min-h-auto min-w-auto gap-y-3 z-3 md:items-center md:justify-normal md:min-h-0 md:min-w-0">
        <div className="text-white text-sm items-center bg-[linear-gradient(rgb(24,25,41),rgb(2,2,5))] shadow-[rgba(86,86,229,0.2)_0px_0px_45.1px_0px] box-border caret-transparent flex h-10 justify-center leading-[22.4px] min-h-auto min-w-auto w-10 border border-gray-800 ml-0 mt-0 p-2 rounded-[100px] border-solid md:text-lg md:h-[70px] md:leading-[28.8px] md:ml-[-5px] md:min-h-0 md:min-w-0 md:w-[70px] md:mt-10 md:p-0">
          03
        </div>
        <div className="items-center box-border caret-transparent gap-x-1.5 flex flex-col justify-between mt-[-63px] max-w-full min-h-auto min-w-auto gap-y-1.5 text-center w-full py-0 rounded-[20px] md:items-center md:gap-x-[27px] md:max-w-[350px] md:min-h-0 md:min-w-0 md:gap-y-[27px] md:text-start md:mt-0 md:py-[30px]">
          <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-start">
            <div className="items-center box-border caret-transparent gap-x-[17px] flex flex-row justify-start gap-y-[17px] text-center md:items-center md:gap-x-[21px] md:flex-col md:justify-between md:gap-y-[21px] md:text-start">
              <div className="text-xs bg-indigo-600 shadow-[rgba(0,0,0,0.12)_0px_4.433px_8.865px_0px] box-border caret-transparent hidden tracking-[-0.569px] leading-[18.6px] text-center uppercase px-[13px] py-2 rounded-[8.8px] md:text-[14.235px] md:flex md:leading-[22.0643px] md:text-start">
                <div className="text-xs box-border caret-transparent leading-[18.6px] text-center md:text-[14.235px] md:leading-[22.0643px] md:text-start">
                  editing
                </div>
              </div>
              <h3 className="text-white text-[21px] font-medium box-border caret-transparent tracking-[-1.12px] leading-[25.2px] min-h-auto min-w-auto text-center capitalize font-secondary md:text-[34px] md:font-semibold md:leading-[44.2px] md:min-h-0 md:min-w-0 md:text-start">
                Editing the Video
              </h3>
            </div>
          </div>
          <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-center">
            <p className="text-white/50 box-border caret-transparent leading-[20.58px] max-w-[238px] text-center w-full mb-2.5">
              Generate views and sales with our expert team, who have worked
              with the biggest creators.
            </p>
          </div>
        </div>
        <div className="relative items-start box-border caret-transparent gap-x-2.5 flex h-auto justify-center max-w-full min-h-auto min-w-auto gap-y-2.5 w-auto z-1 left-[10%] top-[0%] md:gap-x-0 md:h-[500px] md:justify-start md:min-h-0 md:min-w-0 md:gap-y-0 md:top-[-4%] md:w-[400px] md:left-auto">
          <div className="relative items-center box-border caret-transparent flex h-[300px] justify-center min-h-auto min-w-[300px] w-[400px] md:items-end md:h-auto md:min-h-0 md:min-w-[450px] md:w-[600px]">
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36b3452df31baf9345_Glow.avif"
              sizes="(max-width: 726px) 100vw, 726px"
              alt=""
              className="relative box-border caret-transparent h-[120%] max-w-[120%] min-h-auto min-w-auto object-cover translate-y-[-52px] w-[120%] z-2 md:min-h-0 md:min-w-0 md:transform-none"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b73326861ecd43aa1089d3_tag_3.avif"
              alt=""
              className="absolute shadow-[rgba(62,38,196,0.55)_7px_19px_20px_-1px] box-border caret-transparent h-[250px] max-w-full object-cover z-3 rounded-[100px] left-[14%] top-[5%] md:h-[300px]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd368a7f0840c7932011_icon_1.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-full w-[170px] z-3 left-[32%] top-[19%] md:top-[29%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd354eb81fe4d982b5df_video_editing_canvas.avif"
              alt=""
              className="absolute box-border caret-transparent hidden h-[210px] max-w-full z-4 right-[2%] bottom-[11%] md:block md:bottom-[16%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36ed14bc9ea8341c97_icon_2.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-full w-[110px] z-2 left-[58%] right-1/4 top-[24%] md:z-3 md:left-auto md:top-[39%]"
            />
            <div className="absolute bg-[linear-gradient(rgba(255,255,255,0)_37%,rgba(255,255,255,0)_52%,rgba(9,9,9,0.76)_86%,rgb(9,9,9))] box-border caret-transparent hidden h-full w-full z-99 inset-[0%] md:block md:h-[90%]"></div>
          </div>
        </div>
      </div>
      <div className="relative items-start box-border caret-transparent gap-x-3 flex flex-col justify-between min-h-auto min-w-auto gap-y-3 z-3 md:items-center md:justify-normal md:min-h-0 md:min-w-0">
        <div className="text-white text-sm items-center bg-[linear-gradient(rgb(24,25,41),rgb(2,2,5))] shadow-[rgba(86,86,229,0.2)_0px_0px_45.1px_0px] box-border caret-transparent flex h-10 justify-center leading-[22.4px] min-h-auto min-w-auto w-10 border border-gray-800 ml-0 mt-0 p-2 rounded-[100px] border-solid md:text-lg md:h-[70px] md:leading-[28.8px] md:ml-[-5px] md:min-h-0 md:min-w-0 md:w-[70px] md:mt-10 md:p-0">
          04
        </div>
        <div className="items-center box-border caret-transparent gap-x-1.5 flex flex-col justify-between mt-[-63px] max-w-full min-h-auto min-w-auto gap-y-1.5 text-center w-full py-0 rounded-[20px] md:items-center md:gap-x-[27px] md:max-w-[350px] md:min-h-0 md:min-w-0 md:gap-y-[27px] md:text-start md:mt-0 md:py-[30px]">
          <div className="items-center bg-transparent bg-[radial-gradient(circle_at_50%_0%,rgba(23,23,39,0.38),rgb(9,9,9)_100%,rgb(255,255,255))] box-border caret-transparent gap-x-[15px] flex flex-col justify-end min-h-auto min-w-auto gap-y-[15px] text-center border border-zinc-900 ml-7 mb-5 p-5 rounded-[20px] border-solid md:[align-items:normal] md:bg-transparent md:bg-none md:gap-x-[normal] md:block md:flex-row md:justify-normal md:min-h-0 md:min-w-0 md:gap-y-[normal] md:text-start md:ml-0 md:mb-0 md:p-0 md:rounded-none md:border-0 md:border-none md:border-white/60">
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row  md:min-h-0 md:min-w-0 md:text-start">
              <div className="items-center box-border caret-transparent gap-x-[17px] flex flex-row justify-start gap-y-[17px] text-center md:items-center md:gap-x-[21px] md:flex-col md:justify-between md:gap-y-[21px] md:text-start">
                <div className="text-xs bg-indigo-600 shadow-[rgba(0,0,0,0.12)_0px_4.433px_8.865px_0px] box-border caret-transparent hidden tracking-[-0.569px] leading-[18.6px] text-center uppercase px-[13px] py-2 rounded-[8.8px] md:text-[14.235px] md:flex md:leading-[22.0643px] md:text-start">
                  <div className="text-xs box-border caret-transparent leading-[18.6px] text-center md:text-[14.235px] md:leading-[22.0643px] md:text-start">
                    Thumbnail
                  </div>
                </div>
                <h3 className="text-white text-[21px] font-medium box-border caret-transparent tracking-[-1.12px] leading-[25.2px] min-h-auto min-w-auto text-center capitalize font-secondary md:text-[34px] md:font-semibold md:leading-[44.2px] md:min-h-0 md:min-w-0 md:text-start">
                  creating thumbnail
                </h3>
              </div>
            </div>
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-center">
              <p className="text-white/50 box-border caret-transparent leading-[20.58px] max-w-[238px] md:max-w-full text-center w-full mb-2.5 ">
                Generate views and sales with our expert  team, who have worked
                with the biggest creators.
              </p>
            </div>
          </div>
        </div>
        <div className="relative items-start box-border caret-transparent gap-x-2.5 flex h-[284px] justify-start max-w-full min-h-auto min-w-auto gap-y-2.5 w-[400px] z-1 left-[0%] md:gap-x-0 md:min-h-0 md:min-w-0 md:gap-y-0 md:left-auto">
          <div className="relative items-center box-border caret-transparent flex h-[300px] justify-center min-h-auto min-w-auto w-auto md:items-end md:h-auto md:min-h-0 md:min-w-[500px] md:w-[500px]">
            <div className="absolute items-center box-border caret-transparent gap-x-[42px] flex h-full justify-center gap-y-[42px] top-[-7%] w-full z-99 left-6 right-[16%] md:w-[150%] md:left-auto md:top-auto">
              <img
                src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd359cd134e6c0b3088c_01.png"
                sizes="(max-width: 592px) 100vw, 592px"
                alt=""
                className="relative box-border caret-transparent hidden h-[250px] max-h-[250px] max-w-full w-full z-1 rounded-[20px] md:block"
              />
              <img
                src="https://c.animaapp.com/mih7w1om2KrHan/assets/68aab8c5bd9a46b617d22132_image_2_cmp.avif"
                alt=""
                className="relative box-border  caret-transparent h-auto max-h-none max-w-[273.75px] min-h-auto min-w-auto object-contain rotate-[-12.000012571476546deg] w-full z-1 rounded-[15px] md:h-[250px] md:max-h-[250px] md:max-w-full md:min-h-0 md:min-w-0 md:object-cover md:transform-none md:rounded-[20px]"
              />
              <img
                src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd3557ed8e973faed181_03.avif"
                alt=""
                className="relative box-border caret-transparent hidden h-[250px] max-h-[250px] max-w-full w-full z-1 rounded-[20px] md:block"
              />
            </div>
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36b3452df31baf9345_Glow.avif"
              sizes="(max-width: 726px) 100vw, 726px"
              alt=""
              className="relative box-border caret-transparent h-[120%] max-w-[120%] min-h-auto min-w-auto object-cover translate-y-[-52px] w-[120%] z-2 md:min-h-0 md:min-w-0 md:transform-none"
            />
          </div>
        </div>
      </div>
      <div className="relative items-start box-border caret-transparent gap-x-3 flex flex-col justify-between min-h-auto min-w-auto gap-y-3 z-3 md:items-center md:justify-normal md:min-h-0 md:min-w-0">
        <div className="text-white text-sm items-center bg-[linear-gradient(rgb(24,25,41),rgb(2,2,5))] shadow-[rgba(86,86,229,0.2)_0px_0px_45.1px_0px] box-border caret-transparent flex h-10 justify-center leading-[22.4px] min-h-auto min-w-auto w-10 border border-gray-800 ml-0 -mt-6 p-2 rounded-[100px] border-solid md:text-lg md:h-[70px] md:leading-[28.8px] md:ml-[-5px] md:min-h-0 md:min-w-0 md:w-[70px] md:mt-50 md:p-0">
          05
        </div>
        <div className="items-center box-border caret-transparent gap-x-1.5 flex flex-col justify-between mt-[-63px] max-w-full min-h-auto min-w-auto gap-y-1.5 text-center w-full py-0 rounded-[20px] md:items-center md:gap-x-[27px] md:max-w-[350px] md:min-h-0 md:min-w-0 md:gap-y-[27px] md:text-start md:mt-0 md:py-[30px]">
          <div className="items-center bg-transparent bg-[radial-gradient(circle_at_50%_0%,rgba(23,23,39,0.38),rgb(9,9,9)_100%,rgb(255,255,255))] box-border caret-transparent gap-x-[15px] flex flex-col justify-end min-h-auto min-w-auto gap-y-[15px] text-center border border-zinc-900 ml-7 mb-5 p-5 rounded-[20px] border-solid md:[align-items:normal] md:bg-transparent md:bg-none md:gap-x-[normal] md:block md:flex-row md:justify-normal md:min-h-0 md:min-w-0 md:gap-y-[normal] md:text-start md:ml-0 md:mb-0 md:p-0 md:rounded-none md:border-0 md:border-none md:border-white/60">
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-start">
              <div className="items-center box-border caret-transparent gap-x-[17px] flex flex-row justify-start gap-y-[17px] text-center md:items-center md:gap-x-[21px] md:flex-col md:justify-between md:gap-y-[21px] md:text-start">
                <div className="text-xs bg-indigo-600 shadow-[rgba(0,0,0,0.12)_0px_4.433px_8.865px_0px] box-border caret-transparent hidden tracking-[-0.569px] leading-[18.6px] text-center uppercase px-[13px] py-2 rounded-[8.8px] md:text-[14.235px] md:flex md:leading-[22.0643px] md:text-start">
                  <div className="text-xs box-border caret-transparent leading-[18.6px] text-center md:text-[14.235px] md:leading-[22.0643px] md:text-start">
                    post
                  </div>
                </div>
                <h3 className="text-white text-[21px] font-medium box-border caret-transparent tracking-[-1.12px] leading-[25.2px] min-h-auto min-w-auto text-center capitalize font-secondary md:text-[34px] md:font-semibold md:leading-[44.2px] md:min-h-0 md:min-w-0 md:text-start">
                  Post with sEO
                </h3>
              </div>
            </div>
            <div className="items-start box-border caret-transparent flex-col justify-center min-h-auto min-w-auto text-center md:items-center md:flex-row md:min-h-0 md:min-w-0 md:text-center">
              <p className="text-white/50 box-border caret-transparent leading-[20.58px] max-w-[238px] text-center w-full mb-2.5">
                Generate views and sales with our expert  team, who have worked
                with the biggest creators.
              </p>
            </div>
          </div>
        </div>
        <div className="relative items-start box-border caret-transparent gap-x-2.5 flex h-auto justify-center max-w-full min-h-auto min-w-auto gap-y-2.5 w-auto z-1 left-[10%] top-[0%] md:gap-x-0 md:h-[500px] md:justify-start md:min-h-0 md:min-w-0 md:gap-y-0 md:top-[-4%] md:w-[400px] md:left-auto">
          {/*min-h-auto min-w-auto w-auto */}
          <div className="relative items-center box-border caret-transparent flex justify-center  top-[-34px] mt-[30px] left-[8%] min-h-0 min-w-[500px] w-[500px] md:mt-0 md:left-auto md:top-auto">
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b5dd36b3452df31baf9345_Glow.avif"
              sizes="(max-width: 726px) 100vw, 726px"
              alt=""
              className="relative box-border caret-transparent h-[120%] left-[-16%] max-w-[120%] min-h-auto min-w-auto object-cover translate-y-[-52px] w-[120%] z-2 bottom-[38px] md:min-h-0 md:min-w-0 md:transform-none"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6fe2b0c2c7cfd9f95e99a_you_tube_icon.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-full object-cover w-[150px] right-[8%] top-[5%] md:right-[13%] md:top-[10%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b6fddc63aa60dafdeff726_you_tube_icon-1.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-full object-cover w-[90px] left-[8%] bottom-1/4 md:bottom-[37%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/67b731d891c16c44b9310086_Group_1618874258.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-full object-cover w-[120px] z-1 left-[16%] top-[8%]"
            />
            <img
              src="https://c.animaapp.com/mih7w1om2KrHan/assets/68aab7a40dfac4bd3d6c22b8_Glow.avif"
              alt=""
              className="absolute box-border caret-transparent max-w-[350px] object-cover w-full z-6 right-[16%] bottom-[22%] md:max-w-[400px] md:bottom-[26%]"
            />
            <div className="absolute box-border caret-transparent max-w-full w-full right-[26%]">
              <img
                src="https://c.animaapp.com/mih7w1om2KrHan/assets/icon-2.svg"
                alt="Icon"
                className="box-border caret-transparent h-full w-full"
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
